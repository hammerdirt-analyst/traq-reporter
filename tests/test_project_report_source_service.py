...
class ProjectReportSourceServiceTests(unittest.TestCase):
    def test_build_all_groups_trees_and_derives_project_metrics(self) -> None:
        ...
        fixture_inputs = FixtureInputService(
            examples_dir=repo_root / "examples",
            content_dir=repo_root / "content",
        )
        content_source = ContentSourceService(content_dir=repo_root / "content")
        tree_sources = fixture_inputs.load_tree_report_sources()
        ...
        self.assertEqual(briarwood.species_count, 4)
        ...
        self.assertEqual(briarwood.trees[0].main_concerns, ["Extended branches over the parking area"])
